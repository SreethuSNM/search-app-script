...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function performSearch() {
    ...
    try {
      ...
      const searchRes = await fetch(`${base_url}/api/search?query=${encodeURIComponent(query)}&siteName=${siteName}&collections=${collectionsParam}&fields=${fieldsParam}`, { headers });
      ...
      const pageResults = Array.isArray(searchData.pages) ? searchData.pages : [];
      const cmsResults = Array.isArray(searchData.collections) ? searchData.collections : [];
